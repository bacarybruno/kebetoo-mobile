// -- app reducer --
export const SET_THEME = 'app:set_theme'
export const TOGGLE_THEME = 'app:toggle_theme'
export const SET_LOCALE = 'app:set_locale'

// -- user reducers --
export const SET_DISPLAY_NAME = 'user:set_display_name'

// -- posts --
export const API_FETCH_POSTS = 'sagas:api_fetch_posts'
export const API_FETCH_POSTS_SUCCESS = 'sagas:api_fetch_posts_success'
export const API_FETCH_POSTS_ERROR = 'sagas:api_fetch_posts_error'
export const API_FETCH_AUTHORS = 'sagas:api_fetch_authors'
export const API_FETCH_AUTHORS_SUCCESS = 'sagas:api_fetch_authors_success'
export const API_FETCH_AUTHORS_ERROR = 'sagas:api_fetch_authors_error'
export const API_REACT_POST = 'sagas:api_react_post'
export const API_CREATE_REACTION_SUCCESS = 'sagas:api_create_reaction_success'
export const API_EDIT_REACTION_SUCCESS = 'sagas:api_edit_reaction_success'
export const API_DELETE_REACTION_SUCCESS = 'sagas:api_delete_reaction_success'
export const API_REACT_POST_ERROR = 'sagas:api_react_post_error'
export const REPLACE_POSTS = 'sagas:replace_posts'
export const API_FETCH_REACTIONS_SUCCESS = 'sagas:api_fetch_reactions_success'
export const API_FETCH_COMMENTS_SUCCESS = 'sagas:api_fetch_comments_success'
export const COMMENT_POST = 'sagas:api_comment_post'
export const COMMENT_POST_SUCCESS = 'sagas:api_comment_post_success'
export const COMMENT_POST_ERROR = 'sagas:api_comment_post_error'
export const SET_USER_STATS = 'reducer:set_user_stats'
